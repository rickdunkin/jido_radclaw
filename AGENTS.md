...
mix ui.build                           # argus SPA → priv/static/argus/ (install+codegen+build; run before mix release / docker build)
mix jidoclaw.graphql.schema            # regenerate the SDL golden (ui/schema.graphql)
mix jidoclaw.graphql.schema.check      # SDL golden drift guard (in precommit)
```
...
**UI** (`ui/` — the argus SPA, node-served at `/argus`; node >= 24 + pnpm 11, pinned in `mise.toml`; NOT part of `mix precommit`, which stays node-free):
...
The UI gate — run whenever `ui/` is touched: `codegen && check && test && build`, all four (`build`'s `tsc -b` catches what `check`'s type pass allows, e.g. non-erasable syntax). The committed `ui/schema.graphql` golden is written only by `mix jidoclaw.graphql.schema` and is oxfmt-ignored — never hand-edit or reformat it. Generated `src/routeTree.gen.ts` and `src/gql/` are git-ignored; if they're missing, `check` fails loudly and `codegen` recovers them.

Every gateway node serves the built SPA itself: `mix ui.build` (install + codegen + build, offline-safe — codegen reads only the committed golden) emits gitignored `priv/static/argus/`, `Plug.Static` serves the hashed assets, and the `/argus` catch-all (`ArgusController`) returns the shell for deep links — or an honest 404 with the build hint when no build exists. Vite's base is `/argus/` everywhere (dev included), and the SPA router's basepath follows via `import.meta.env.BASE_URL`.

**Prerequisites**: Elixir >= 1.17, OTP >= 27, PostgreSQL. Ollama recommended for local dev. Node.js >= 24 + pnpm only for `ui/` work (`mise install` provides both).
...
**Known limitations** (anubis_mcp 1.6.2 + jido_mcp + jido_action — patched in `lib/jido_claw/core/`):
...
- Runtime patch overrides `Jido.MCP.Server.Runtime` to route its two tool error arms through `JidoClaw.MCPServer.ErrorBoundary` (PD1-2) — the public server emits the dual-content structured envelope — `content[0]` byte-identical legacy inspect text, `content[1]` the closed-registry JSON envelope — while every other server (consolidator, deposit) keeps the byte-identical legacy single-item arm — and to mint + thread the per-call wrap-provenance token (next bullet). Remove once jido_mcp offers a per-server error-rendering seam. Registry, boundary, reduction tiers → [docs/system/mcp-server-surface.md](docs/system/mcp-server-surface.md)
- Compile-time GENERATED fork of jido_action 2.3.1's `Jido.Exec` (`lib/jido_claw/core/jido_exec_patch.ex`; Apache-2.0 — license + §4(b) notice in `priv/licenses/`, embedded for the escript behind `jidoclaw --third-party-licenses`): the exec map-wrap of a tool error is lossy, so the boundary's tier-1 selection ("this IS our envelope, report its domain code") was shape-inferred and forgeable — a hand-built native error whose details carried `:code`+`:details` mis-tiered into tier 1. The fork's two map-wrap arms stamp an opt-gated `Map.put_new` per-call reference, ONLY when the pre-wrap reason matched the raw canonical-envelope contract; the boundary detaches it on exact ref identity and tier 1 REQUIRES the witness; only the public server's runtime path mints. Every no-opt exec caller is byte-identical by construction, and a package-pinned drift guard (mix.lock version + outer checksum + source sha, compile-time gated) fails loud on any jido_action bump. Remove once jido_action stamps wrap provenance natively or exposes a result-normalization seam. Port map: `docs/exploration/pms/pad/PORT-PD1-2-EXEC.md`; mechanics → [docs/system/mcp-server-surface.md](docs/system/mcp-server-surface.md)
- Because Elixir has no per-warning suppression, the `precommit` gate runs **`mix jidoclaw.compile_check`** instead of `compile --warnings-as-errors`: it clean-recompiles and fails on any warning/error **except** an explicit, documented allowlist. The allowlist (in `lib/mix/tasks/jidoclaw.compile_check.ex`) is **currently empty** — `PullRequestCoordinator.do_attempt/5`'s retry/abort `else` branches are now live (its helpers do real, fallible work: `generate_patch/3` carries a terminal `{:generation_failed, _}` and `JidoClaw.GitHub.PatchQuality.validate/1` a retryable `{:quality_failed, _}`), so the two former dead-`else` warnings are gone at the source. The mechanism remains for genuinely-unavoidable warnings (upstream-generated code or intentional scaffolding); add an entry only when justified inline and re-check it on every dep bump / Elixir upgrade. Because the gate hand-sequences compile tasks, it re-runs the `jidoclaw_release_patches` compiler before `compile.app` and self-asserts the fork BEAM (`assert_exec_fork!/2`), so its clean sequence can never drop the generated `Jido.Exec` fork.
...
- **Run-Failure Taxonomy (retryable ≠ resume-unsafe)**: `JidoClaw.Orchestration.RunFailure` is the one closed 22-kind vocabulary for *why an agent run failed* — platform-vs-agent provenance in the name, policy derived from set membership, never string-sniffed at decision sites. `classify/1` and `format_reason/2` are TOTAL (exceptions, throws, exits all fall back to `:agent_unknown` / a bounded string); Splode containers classify order-invariantly through an explicit total precedence rank; `retryable?/1` (retry-the-WORK) and `resume_unsafe?/1` (abandon-the-CONVERSATION anchor) are **independent predicates** whose overlap (semantic-inactivity, session-poisoned: retry fresh, never on the old anchor) is the point; `failure?/1` is false ONLY for `:user_cancelled`. It sits ABOVE the pinned `Forge.Error.classify/1` `{kind, recovery}` contract (composes, never breaks) and consumers enrich, never redecide — harness `:error` broadcasts carry `kind`, the composer Lane-B threads it into the non-durable Trace only, and `error_details/2` policy bits strip caller overrides in both key forms. The narrow retryable set is a deliberate consumer policy seam. Kind table with producer status, precedence, string rules, residuals → [docs/system/run-failure.md](docs/system/run-failure.md)
...
- **Forge Session Resume (native CLI resume, fenced recovery)**: vendor runners opt into `runner_config[:resume] :: :off (default, byte-identical argv — PR-3 pins hold; the executor never arms) | :armed` — claude mints its anchor CLIENT-side (`--session-id` pre-spawn through the fenced writer seam, id-verified against the CLI echo), codex captures `thread.started` as provisional-until-a-real-`turn.completed`-terminal (CH2-6 — a terminal-less exit-0 stream stays provisional); continuations send `--resume <id>`/`resume <id>` with a GUIDANCE-ONLY prompt (never the task twice, never `--continue` — the `:guidance` opt is read only by armed continuations, so a fresh turn structurally carries the full task), a resume-unsafe failure POISONS the anchor (sticky, never reused), and **runners never auto-retry**. Every claimed session carries the HARNESS-level incarnation fence `metadata["forge_recovery"]` `{epoch, token, current_checkpoint_id, recovery_degraded}` — minted CAS-once at claim (terminal reuse re-mints blank), the token authorizes writes only, and stale writers get `{:error, :stale_resume_write}` and write NOTHING. The `current_checkpoint_id` pointer (moved only by the CHECKED pointer-first save) is the ONLY recovery-selection authority; recovery transplants the newest `{epoch, revision}` copy under the mint's lock, and the checked initial checkpoint lands before every `:ready` (failure ⇒ honest `recovery_degraded`, self-healed by the next checked save). The consolidator is the first multi-iteration driver: attempt-bound tokenized MCP capabilities (closed tokens get typed refusals, readers included), reserve-then-execute effect ledger, commit-as-marker + clean-exit publish gate with the `:run_id`-pinned certificate row and three-outcome reconciliation (`publish_outcome_unknown` never republishes), one ledger-gated fresh retry, one effect-free crash replay (marked `source: :replay`, EM3-3). Host-tier CLI teardown is sequenced + identity-verified (`ChildTracker` tagged incarnations → `OsCmd.terminate_tree/2` TERM→grace→verified KILL; the session-wide barrier runs before `run_forge_home` — the LAST resource — is removed), and the claude env denylist can never be re-opened by operator allowlists. Crash-native resume is `:local`-only (docker = in-run continuations, fresh via the cwd-gate after a crash). Fencing laws, argv tables, codecs, ledger table, teardown mechanics, residuals → [docs/system/forge-session-resume.md](docs/system/forge-session-resume.md)
...
- **GraphQL Read Surface (argus P1 — read-only /gql, SDL golden)**: `JidoClaw.Web.GraphQL.Schema` serves AshGraphql queries over `Projects.Project` + `Orchestration.WorkflowRun` behind `[:api, :api_auth, :graphql]`, where the `:graphql` pipeline is `GraphqlBatchGuard` (transport batching rejected 400, presence-based on the `_json`/`operations` keys across all three absinthe ingress vectors — the complexity/token limits are per batch element, so a batch's aggregate cost would be unbounded) then `GraphqlTenantGate` (an activity GATE: inactive tenant → 403, infra failure → 503, malformed actor fails closed — Project is a global resource, so the gate is what stops a suspended tenant's valid key; its `ensure_active` may provision a missing tenant row — the one write behind the surface) then `AshGraphql.Plug`. **No GraphQL mutations exist**; field exposure is a positive `show_fields` allowlist (cloak/lease/payload columns absent by construction, identifier-level deny-list in tests); `derive_filter?`/`derive_sort?` are false — ordering + validated `limit` (1..200, default 50) live in the `:alphabetical`/`:recent` read actions; `disposition`/`findings_deferred_count` calculations delegate to the same `Visibility` derivations `run_view/3` uses (camus C1-4 on GraphQL from day one); complexity/token controls and the batch rejection are pinned on the forward/pipeline and route-test-proven. The committed `ui/schema.graphql` golden has ONE writer (`mix jidoclaw.graphql.schema`) and a precommit drift guard (`mix jidoclaw.graphql.schema.check`); dev/test-only `/graphiql` sits behind an interface-only guard (GraphiQL executes `?query=` documents on HTML GETs — only a bodyless, query-less HTML GET passes). Mechanics, enum/status type move, guard semantics, residuals → [docs/system/graphql-surface.md](docs/system/graphql-surface.md)
- **Channels Surface (argus P4 — key-only ArgusSocket + workflows:run topics)**: `JidoClaw.Web.ArgusSocket` (`/argus/ws`) is a SEPARATE socket from `UserSocket` — capability separation by construction: the SPA's baked key can never reach the mutable `rpc:*` surface, and the boundary is test-pinned both ways. Auth is the `authToken` header transport only (endpoint-level `auth_token: true` — MUST stay socket-level, the endpoint macro clobbers a nested `websocket: [auth_token: ...]`; the key never rides the URL, so no filter_parameters change), through the shared `ApiKeyAuth.authenticate_api_key/1` (one `:auth_event` per attempt — a token-less connect emits its row via `ApiKeyAuth.missing_api_key_failure/0`) then the `/gql`-mirroring tenant activity gate (inactive/infra → refuse, fail closed). `WorkflowsChannel` joins are canonicalize → subscribe → policy read (uppercase UUIDs would subscribe a dead topic; subscribe-before-read so no terminal is lost; uniform `"not_found"` for cross-tenant/suspended/nonexistent, `"unavailable"` for infra) and push MINIMAL `run_event` payloads — `%{id, kind}`, id bound to the authorized run, kind allowlisted to the five lifecycle broadcasts (the inventory is centralized in `RunPubSub.lifecycle_kinds/0`), the info map never forwarded; the client refetches via GraphQL. Composer runs publish start + terminals themselves (they never ride `ReactorMiddleware`), durable-then-notify, mapping each `route_*` terminal onto its status-family wire kind — the set stays exactly five. Join replies carry `%{id, status}` (lowercase-snake wire casing). Suspension force-disconnects the tenant's sockets via a direct endpoint-independent PubSub broadcast from the tenant manager; the SPA's socket wrapper parks itself after a bounded run of refused reconnects (audit-flood guard) with a manual Retry. Key revocation on a live socket is revalidated on transport reconnect only (documented residual). Join mechanics, wire contract, client wrapper, residuals → [docs/system/channels-surface.md](docs/system/channels-surface.md)
