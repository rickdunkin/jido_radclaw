defmodule JidoClaw.Agent.Workers.SketchReviewer do
  ...
  use JidoClaw.Agent.Defaults,
    ...
    description:
      "Reviews a throwaway prototype in an isolated sandbox for logic and edge-case correctness. Read-only file tools (plus read-only access to the real project tree) — never runs commands or touches git. Return a structured review (`overall`, short `summary`, an `action_needed` line, and a list of `findings`, each with a short stable `title`, `severity` info/warning/error, `confidence` likely/unsure, `location`, and `description`).",
    tools: [
