...
- **Conditional:** H4/H5 (network/cluster) apply only when `:cluster_enabled` is true; H6 (trust gaming) is reachable by the LLM via the solution store even single-user. *(All three since fixed 2026-06-11 — see the per-finding notes.)*
...
### H4. Unauthenticated solution injection over clustered PubSub ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** New `JidoClaw.Network.PeerDirectory` allowlist (`JIDOCLAW_NETWORK_PEERS` comma-separated base64 Ed25519 pubkeys, or the `:network_peer_keys` app-env test seam; read at call time so `.env` works, on the `AdminAccess` pattern). Agent ids are always *derived* via `Identity.derive_agent_id/1` — operators configure keys only, never id:key pairs; invalid entries warn-and-skip so one bad key doesn't kill all inbound, and `configured?/0` is true only when at least one key actually parsed. The stub is replaced by the `Protocol.verify_and_normalize/3` boundary gating **all three** inbound paths (`:solution_shared` and the previously completely ungated `:solution_requested`/`:solution_response`): envelope decode + validation, per-handler expected-type assertion, `PeerDirectory.fetch(from)` key lookup, Ed25519 signature verification, then payload canonicalization via JSON round-trip — a same-day post-review hardening: clustered PubSub delivers raw BEAM terms (atom keys, structs, tuples — shapes JSON decoding could never produce), and the signature only proves the payload *encodes* to the signed bytes, so handlers must consume the round-tripped plain data, never the raw term. Drop means no store, no response broadcast, no `add_peer`, no `Reputation.record_share` (`:bad_signature` logs at warning, `:unknown_peer` at debug for noise control on shared segments). The request handler also hardens peer-supplied input end-to-end: `sanitize_request_opts/1` replaces the crash-prone `String.to_existing_atom` atomization with a key+value whitelist (`language`/`framework`/`error_class` binary-only, `limit` integer clamped to 25, `threshold` numeric — closes L18), and the unsigned envelope `request_id` is type-guarded before `Protocol.response_message/3`. Connecting with no peers configured warns that all inbound will be dropped (outbound-only sharing stays legitimate). **Residual risk, accepted and documented in-code:** the signature covers only `payload` — cross-`from` spoofs fail key lookup, cross-type replay is closed by the type assertion + disjoint payload shapes, but verbatim replay of an old signed message remains possible (no nonce/timestamp window); acceptable because PubSub injection requires BEAM cluster membership (H5 secret + cookie), and a cluster member already has full RCE. Covered by `test/jido_claw/network/peer_directory_test.exs` + `node_test.exs` (trusted/unknown/tampered/cross-type-replay × store/request/response, hostile opts keys and values, wire-shape pins).

### H5. libcluster gossip configured with no `secret` ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** The duplicated gossip config (`:gossip` branch + unknown-strategy fallback, which also had a hardcoded port) is DRY'd into one `gossip_topology/0` that always includes `secret: gossip_secret!()` — sourced from `config :jido_claw, :cluster_secret` or `JIDOCLAW_CLUSTER_SECRET` read at call time (`.env` loads before `cluster_children/0` builds the topology; `runtime.exs` is too early), trimmed, blank = missing. Missing → **raise** with an actionable message naming both config sources and the `cluster_enabled: false` escape hatch (on the `VaultConfig` raise-on-missing precedent); the raise lives in the gossip path only — `:kubernetes`/`:epmd`/`:none` need no secret, and `topology/0` is reached only when `:cluster_enabled` is true (default false), so single-node boots never hit it. No committed default secret. Boot nudge: when clustering is enabled but `Node.self() == :nonode@nohost`, `Application.cluster_children/0` warns that distribution isn't started (pointing at `--name`/`--sname` + non-default cookie). README documents the layered model — the gossip secret *encrypts* discovery (libcluster AES-CBC, no MAC — not authentication), the distribution cookie gates *membership*, peer signatures (H4) authenticate *messages* — plus `RELEASE_COOKIE`/`-setcookie` guidance; `.env.example` gained commented `JIDOCLAW_CLUSTER_SECRET=` / `JIDOCLAW_NETWORK_PEERS=` entries. Covered by `test/jido_claw/core/cluster_test.exs` (secret present/env-var/blank/missing × gossip + fallback, `:none`/`:epmd` unaffected).

### H6. Solution trust score is self-asserted, not earned — gameable on two paths ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** Both layers. Facade: `:trust_score`/`:verification` joined `@forced_inbound_keys` (peer-supplied values are dropped before `Solution.store/1`; content/tags preserved, `sharing` stays forced `:shared`; a same-day post-review hardening (P1) also forced `agent_id` — `store_inbound/3` now takes the signature-verified sender id and drops payload-asserted attribution, string or atom key), and `to_wire/1` no longer transmits either field — receivers force-drop them, so sending them was dead weight inviting a future receiver to trust them. Contract: `:store` no longer `accept`s `:trust_score`/`:verification` — any future caller passing them gets a loud `NoSuchInput` instead of silent self-assertion; new rows keep the attribute defaults (`0.0`/`%{}`) and earn trust via the existing `verify_certificate` → `:update_verification_and_trust` server-side recompute (deliberately **no** `RecomputeTrustScore` on `:store` — it would lift fresh unverified rows to a ~0.5 baseline and change ranking semantics). `:import_legacy` intentionally keeps accepting both (the v0.5.x migrator imports earned trust) — pinned by a contrast test. The caller-less self-assert vectors `:update_trust`/`:update_verification` (and their code_interface defines) are deleted outright. Test plumbing: `solution_fixture/4` no longer injects the two keys; a non-default `:trust_score`/`:verification` override reroutes the create through `import_legacy` (same Redact/FK/embedding-status changes) so ranking fixtures like `hybrid_search_sql_test`'s `trust_score: 0.95` stay exact. Covered by `network_facade_store_inbound_test.exs` (hostile string-keyed payload lands with `trust_score == 0.0` / `verification == %{}`, rest preserved, `agent_id` forced to the verified sender) and `:store`-rejects/`:import_legacy`-accepts contract pins in `solution_test.exs`.

---

## HIGH — secrets at rest / in transit

### H7. Setup writes API keys to `.env` world-readable ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** `persist_env_var/3` now (1) defensively tightens a pre-existing `.env` to `0600`, guarded by `File.lstat` so only a regular file is touched — a symlink or directory sitting at the path keeps its own mode (`stat` would follow the symlink); (2) writes through a unique per-call tmp (`.env.<n>.tmp`) that is `touch`ed and chmod'd `0600` **before** any content lands in it, then renamed over `.env` — rename preserves mode, so there is no world-readable window even transiently and concurrent callers can't trample each other's tmp; (3) wraps the write in `try/after File.rm(tmp)` so a failed write/rename never leaves a secret-bearing tmp behind. Covered by the new `test/jido_claw/cli/setup_test.exs`: fresh-create at 0600, upsert preserving comments/order while tightening 0644→0600, and a forced rename failure pinning tmp cleanup plus the lstat guard (a directory at `.env` keeps its mode).

### H8. Conversation migration imports content with no redaction ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** `:import` now runs `Changes.RedactContent` — the same idempotent `Transcript.redact/1` hook `:append` uses, covering both `content` and `metadata` — registered ahead of `ValidateCrossTenantFk` (matching Solution's `:import_legacy` ordering). Fixed at the action, not the migrator, so every future `Message.import/1` caller is covered; migrator re-runs converge (redaction is idempotent) and `import_hash` dedup is unaffected (the hash is computed from the raw line). The aggravating-factor test was made honest: the fixture now carries a **raw** `sk-...` key and the export test asserts the fixture is unscrubbed before import — the redaction-manifest position assertions now pass *because of* the fix instead of trivially. A new unit test in `message_test.exs` re-reads the imported row from Postgres and pins persisted content `[REDACTED:API_KEY]` + metadata `[REDACTED]`.

### H9. Forge's secret env-scrubbing denylist has exploitable gaps ✓ verified — ✅ fixed 2026-06-11
...
**Fixed (2026-06-11):** Took the allowlist option. `scrubbed_cmd_env/1` / `scrubbed_port_env/1` now unset every parent var **not** on an explicit inheritance allowlist: shell/locale ergonomics (`PATH`, `HOME`, `TERM`, `LC_*`, `XDG_*`, ...), TLS trust roots (`SSL_CERT_FILE`, `CURL_CA_BUNDLE`, ...), and proxy config — `HTTP(S)_PROXY`-style vars inherited only when the value carries no userinfo: any `@` in the authority drops the var, with or without a password (a 2026-06-11 follow-up review caught that the original check required a `:` in the userinfo, so token-only `http://token@proxy:8080` and scheme-less `token@proxy:8080` were inherited; an `@` past the authority, in a path or query, still doesn't count). Credential-*capability* vars (`SSH_AUTH_SOCK`, `GPG_TTY`, `GNUPGHOME`) are deliberately excluded by default (SSH-based `git clone` in the resource provisioner and GPG-signed commits need an explicit opt-in); operators extend the allowlist via `JIDOCLAW_EXTRA_ALLOWED_ENV_VARS` (comma-separated, reachable from `.env`) or `config :jido_claw, :extra_allowed_env_vars` / `:extra_allowed_env_prefixes`, read at call time. Output shape is unchanged (unset tuples ++ overrides), so all consumer call sites work untouched and explicit overrides still win. `sensitive_key?/1` (which keeps feeding `redact_value/2`, `Transcript`, and the M2 log filter) gained `_CREDENTIAL`/`_CREDENTIALS` suffixes plus specific `AWS_ACCESS_KEY_ID`, `SECRET_KEY_BASE`, `ONECLI_AGENT_TOKENS` — but **not** blanket `_TOKENS`/`_BASE` suffixes: `input_tokens`/`output_tokens`/`total_tokens` are ubiquitous LLM usage counters (the `_TOKENS` variant was tried and broke four tool/view tests by masking counters to `[REDACTED]`), and `URL_BASE`/`API_BASE` are config; under the allowlist the inheritance boundary no longer depends on key naming anyway. Bonus footgun closed: `Core.OsCmd.run/3`'s `:env` now defaults to `Env.scrubbed_cmd_env()` instead of `[]` (inherit-everything) when the caller forgets the option; an explicit `:env` is used as-is.
...
- **M1. `Memory.Block` content is never redacted** — ✅ fixed 2026-06-11. `lib/jido_claw/memory/resources/block.ex:134-153`. `Fact.record` runs `RedactContent` (fact.ex:177); Block's `:write` runs validation/cap changes only (✓ verified). Blocks are the one tier rendered verbatim into the system prompt every turn (`agent/prompt.ex:263`), so a secret curated into a block reaches the provider in plaintext. `Security.Redaction.Memory`'s own moduledoc says it covers Blocks. Add `RedactContent` to `:write` and the revise path. **Fixed:** `:write` now runs an inline `RedactContent` (mirroring Fact's) over `value` + `description`, registered **before** `CapValueLength` so the cap validates the redacted value (a placeholder can be longer than a short raw secret — pinned by a 31-byte-raw / 18-byte-redacted / 25-cap test); consolidator, CLI, and `revise/3` all converge on `:write`, and `:invalidate` accepts no content, so coverage is complete and the `Security.Redaction.Memory` moduledoc claim is now true. A prompt-level regression test (`prompt_snapshot_test.exs`) pins that a secret written to a block never reaches `Prompt.build_snapshot/2` output. Two adjacent gaps closed while here: the CLI cross-scope block override built `source: :user_save` (Fact vocabulary — Block's `@sources` constraint rejected the write; now `source: :user`, with a regression test), and Fact's previously-untested `:record` redaction wiring gained a test.
- **M2. Logger *metadata* is never redacted** — ✅ fixed 2026-06-11. `lib/jido_claw/security/redaction/log_redactor.ex:18-35`. The filter rewrites only `event.msg`; `Logger.metadata(api_key: key)` or structured Logger calls pass secrets to backends unscrubbed. Map over `event.meta` too, keyed by `Redaction.Env.sensitive_key?/1`. **Fixed:** the filter now rewrites `event.meta` and the previously-unhandled `{:report, map|keyword}` / `{format, args}` msg shapes (OTP crash reports dumping GenServer state were the worst leak). Standard logger keys (`mfa`, `file`, `line`, ...) skip verbatim — `crash_reason` deliberately does **not**. Sensitive keys (atom *and* binary, via `Env.sensitive_key?/1`) mask to `[REDACTED]`; other values go through a bounded shape-preserving walker — maps/keyword lists keyed, tuples reconstructed (`{:badmatch, %{api_key: ...}}` shapes), structs rebuilt field-wise, charlists redacted as text via a binary round-trip (originally opaque pass-through — a 2026-06-11 follow-up review caught that `~s` renders charlists, so a charlist format arg leaked raw; benign charlists still pass byte-identical and non-codepoint lists pass through rather than dropping the event), improper lists safe — that **fails closed** at the depth bound: containers nested past 4 levels become `[REDACTED:DEPTH]`, never passed through. Format args are walked with the same machinery (`~p` map args keep their shape so formatting is unaffected); format strings are redacted directive-safely by splitting on the full `~F.P.PadModC` grammar (incl. `t`/`l`/`k` modifiers, `#`/`+` integer controls, `~~`) and scrubbing only literal segments. Since :logger *removes* a crashing filter (permanent fail-open for all subsequent events), the filter body is wrapped in a catch that drops the offending event (`:stop`) instead.
...
- **L18. Network request opts atomized then merged into Matcher options** — `lib/jido_claw/network/node.ex:315-327`. `String.to_existing_atom` is rescue-guarded (no atom DoS), and `scope_opts` append-order protects tenancy, but a peer can inject arbitrary *known* option atoms. Whitelist permitted request-option keys. *✅ Closed 2026-06-11 by the H4 fix: `sanitize_request_opts/1` is a key+value whitelist (`language`/`framework`/`error_class` binary-only, `limit` integer clamped, `threshold` numeric; everything else dropped) — it also fixed the latent `FunctionClauseError` on non-binary opt keys that the `ArgumentError` rescue missed, and scope opts now precede peer opts in the Keyword merge.*
...
3. ~~**H7 + H8 + H9 + M1 + M2** — the secrets cluster (chmod, import redaction, env allowlist, block redaction, logger metadata).~~ ✅ Done 2026-06-11 — all five fixed (see the per-finding notes): `.env` created/tightened to 0600 with atomic 0600 tmp writes; `:import` redaction with a now-honest raw-secret fixture; allowlist-based child env inheritance plus a scrubbed `OsCmd.run/3` default; Block `:write` redaction ordered ahead of the length cap (and the latent CLI `source: :user_save` bug); logger meta/report/format-args redaction with a fail-closed depth bound.
...
5. ~~**H4 + H5 + H6** — before ever enabling clustering: peer verification, gossip secret, trust-score hardening.~~ ✅ Done 2026-06-11 — all three fixed (see the per-finding notes): `PeerDirectory` allowlist + `verify_inbound/2` drop-by-default gating on all three inbound paths (also closing L18); gossip secret required with raise-at-boot + layered-trust docs; trust stripped at the facade, rejected by `:store`, dead self-assert actions deleted.
...
| infra/network | Supervision tree careful; webhook textbook | Network layer had stubbed security boundaries (H4/H5 — since fixed, with peer-directory + node gating tests) |
