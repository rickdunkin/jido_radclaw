...
*Approved 2026-07-12. Executes **#16 only** from
[the pre-argus do-now queue](../pre-argus-do-now/README.md): the served-MCP
boundary error-code registry (pad PD1-2) — camus C1-3's closed-contract
posture applied to the tool surface. Lands commit-ready but uncommitted;
`mix precommit` green is the completion bar; greenfield — no compat shims.*

**Status (2026-07-12): BUILT** — all sections §1–§7 landed in one session
against HEAD `57f61037` (the two commits past the plan's verify sha
`91e355ee` are ui/.claude-only, as predicted). 51 codes registered, surface
v1.3, golden fixture regenerated, `mix precommit` green. Deviations below
are the authoritative build-vs-plan record; Status lines landed on
PD-FIRST-WAVE item 2, pad PD1-2 + OQ-2, and pre-argus README §16 + §4.

## Context

Scope history: the six-item Wave E plan proved too large, then the #16+#17
cut did too; **#17 is now deferred** alongside #18–#21 (the prior drafts
remain their references:
...
them — no dependency is severed by the split.
...
plus thirteen further review rounds on this reduced cut, same day): errors
...
- **Nothing committed by the agent**; work lands unstaged, ending with
  files-to-stage plus a suggested commit slicing. Completion bar:
...
metadata). Both stages are needed: `sanitize_details/1` alone is not a
total arbitrary-reason sanitizer (a bare atom arg returns `%{}`
(error.ex:285), improper lists raise, hostile structs can invoke
`Inspect` early), and already-canonical envelopes bypass `normalize/1`'s
sanitization (error.ex:119-122) while a raw Reactor reason can be
aggregate-huge. Fidelity note: `content[0]` shows the POST-normalization
envelope (byte-identity holds against the normalized legacy arm) — the
raw uncapped Reactor term is deliberately not preserved.
...
halt directives, telemetry text, or logs. `reason_head` + exact `skill`
split `{:exit, :timeout}` from `{:exit, :killed}` and equal-length skill
names even when message prefixes collide; secrets of ANY length hit the
constant marker — same failure class. The fingerprint only ADDS
distinction beside the message component, so no previously-distinct
signatures merge. Identical failures keep identical signatures (the
determinism genuine loop detection needs): pids/refs in details
contribute class markers, never values — no new drift — and
message-embedded pid TEXT was already unstable at HEAD, so signature
stability strictly does not regress. §6 pins distinctness, determinism,
runtime-identity stability, and the secret regression; loop-guard.md
gets the same-change docs bump (§7).
...
- **The entire canonical envelope is normalized through the budgeted
  ...
  here would convert a tool error into a JSON-RPC execution failure.
  `JsonSafe` is NOT total today (verified): invalid-UTF-8 binaries pass
  through the binary clause (json_safe.ex:87-89) and `<<255>>` produces
...
- **Bounded WORK, not just bounded output — a shared budgeted walker**:
  ...
  tagged encoding draws) — and a **pinned result contract**:
  ...
  marker path. It backs every unbounded-input site: the boundary's
  envelope production trips → straight to the reduced envelope (which
  extracts only allowlisted keys — bounded by construction, no full
  walk); run_skill's reason encoding (§2) trips → a constant truncation
  marker + `observed_at_least`; LoopGuard's structural projection (§2)
  trips → a constant budget-exceeded sentinel component.
  **Size-metadata semantics**: `original_byte_size` appears ONLY on
  fully-traversed over-cap values; budget-tripped reductions carry
...
- **Aggregate byte cap on the structured item**: `JsonSafe` ensures
  ...
  forms — first found wins; `retry` is the downstream-retry signal — the
  in-call gate consumed the producer flag before the boundary ran — and
  the `available` pair is the whole point of a hint envelope), **each
...
- `run_skill.ex`: unknown skill (today a plain string from
  ...
  redaction) so the identity projection covers this producer too.
  ReactorRunner failures — the THREE-tuple arm `{:error, reason, _run}`
  at :78, the only arm `ReactorRunner.run/3` feeds — get §2's
  normalization; the TWO-tuple arm at :80 (Skills.get/Compiler failures)
  special-cases the unknown skill into `:unknown_skill` and otherwise
  stays an open forwarder (§1's `@open_forwarders`).
...
- `docs/system/mcp-server-surface.md`: closed-error-contract invariant
  ...
  scoping to the public server, hints); the runtime patch documented
  beside the anubis patch; `sources:` += error_codes.ex,
  error_boundary.ex, jido_mcp_runtime_patch.ex, the sweep test;
  `verified:`/`verified_sha` bump. AGENTS.md known-limitations gains the
  patch line (same commit).
...
*(Maintained as work proceeds: what the plan assumed, what the code
revealed, what was chosen and why, forced vs surfaced.)*

- **The `fixed_shape_map` smell group needed run_command.ex pinned too**
  (forced; 2026-07-12). Reach groups the `{code, message, details}` wire
  shape across producers and fails only the group's ANCHOR file — which
  proved unstable across runs once the new producers joined the group
  (one run anchored network_share, a later one run_command, another
  passed entirely). All new envelope producers (network_share, run_skill,
  list_directory) carry the house `reach:disable-for-this-file
  fixed_shape_map` pragma with the Tools.Error justification, and
  run_command.ex — the one pre-existing group member without it — is now
  pinned as well, so the gate is deterministic regardless of anchor
  choice.

- **JsonSafe totality broke the Lua runner's accidental refusal mechanism**
  (forced; 2026-07-12). The plan called JsonSafe's strengthening "safe for
  its existing consumers", but `:lua_result_not_encodable` was implemented
  BY the old non-totality: the runner JsonSafe'd the result and let
  `Jason.encode!` raise on invalid UTF-8. With binaries now
  replacement-char scrubbed, that path silently returned mutated data —
  violating the pinned "refused honestly" contract
  (docs/system/lua-code-mode.md). Fix: the runner owns an explicit
  recursive `utf8_clean?/1` scan over the RAW results before JsonSafe and
  refuses on failure — same wire envelope, now honest by construction
  instead of by accident; the `measure/1` `:unencodable` arm stays as the
  residual backstop.

- **Producer normalizers exposed as documented test seams** (surfaced;
  2026-07-12). The plan's producer tests (§6) need the `:skill_cancelled`/
  `:skill_run_failed` and `network_share` normalization arms driven
  directly, but the runner path requires a live compiled-skill run and the
  network path a connected P2P node — both heavyweight and flaky as test
  fixtures. Following the file's own `scope_context/1` "Test seam"
  precedent, `RunSkill.unknown_skill_envelope/1` +
  `RunSkill.runner_failure_envelope/2` and
  `NetworkShare.normalize_share_result/2` are public with `@doc`/`@spec`;
  the full `run/2` paths are still driven where cheap (unknown skill,
  glob envelopes).

- **The minimal envelope tier is reachable only via byte-undercounted
  numeric structures** (surfaced; 2026-07-12). The reduced tier's shared
  budget accounts binary bytes, not numeric width, so ordinary reductions
  are bounded well under the 16 KiB cap — the plan's "reduction STILL over
  the cap" row is exercised with wide-integer lists (900 × 17-digit
  numbers ≈ 16 KiB JSON per field at near-zero accounted bytes). That
  undercount is exactly what the minimal tier exists to catch; no
  constant was retuned.

- **A raising `Inspect` impl never escapes `inspect/1` on modern Elixir**
  (forced; 2026-07-12). The plan's guard rows assumed all three escape
  kinds (raise/throw/exit) reach our rescue+catch; in fact Elixir ≥1.15's
  `Kernel.inspect/1` catches a RAISING impl itself and returns the
  `#Inspect.Error<…>` diagnostic string — valid UTF-8, no escape, and the
  same behavior HEAD's raw runtime arm already had. Only throw and exit
  escape. Consequence: the "raise" rows assert the diagnostic rendering
  (still a decodable tool response, never a protocol error); the exact
  pinned static fallbacks are exercised by the throw and exit rows. The
  guarded regions keep all three kinds covered — raise protection remains
  for OTHER raise sources (malformed structs, serializer bugs). Hostile
  fixtures live in `test/support/hostile_inspect.ex` — protocols are
  consolidated in the test env, so a `defimpl` inside a test file never
  dispatches.

- **Post-review round: bounded-walker key markers + collision sentinel**
  (forced; 2026-07-13). Review found `bencode_key/3`'s fallback ran
  `safe_inspect/1` BEFORE bounding/charging — a hostile/buggy user
  `Inspect` impl on a struct key (or a composite key containing one) could
  allocate unboundedly or never return before any budget saw a byte — and
  the map fold's `List.keysort(0)` over ORIGINAL keys was O(key size)
  comparison work even when keys were never rendered. Fix: composite keys
  (tuple/list/map/struct) and out-of-int64 integers take constant
  `<<key:*>>` class markers (int64-range integers still render exactly;
  floats/runtime identities keep their consolidated, bounded stdlib
  renders), and the fold now streams entries into the result map with a
  `collided` MapSet — encoded-key collisions resolve to the constant,
  once-charged `"[key-collision]"` sentinel, order-independently, instead
  of the term-order winner. `encode/1` (totality, not bounded work) and
  `fproject_*` (already marker-projected) are untouched. One test-shape
  correction: the hostile-struct-KEY wire assertion drives
  `ErrorBoundary.error_response/2` directly instead of the full runtime —
  jido_action's OWN telemetry sanitizer (`Jido.Action.Sanitizer.safe_inspect`,
  no throw handling) crashes on a throwing key `Inspect` before the
  boundary ever sees the envelope on that path; upstream, out of scope.
  The composite-key (tuple-marker) row still rides the full runtime.

- **Post-review round: skill-name type invariant at the loader boundary**
  (forced; 2026-07-13). The 256-byte name bound guarded only
  `is_binary(name)` names — YAML `name: 123`, `name: {a: 1}`, explicit
  `name:` (nil), and `!!binary`-tagged invalid UTF-8 all cached as valid
  skills and crashed downstream (system-prompt interpolation at
  `prompt.ex`, the `/skills` renderer). `build_skill/2` is now a cond
  with the non-binary arm first (logging the TERM CLASS, never the value)
  and a `String.valid?/1` arm deliberately after the byte-size arm (the
  validity scan stays bounded at 256 bytes). One choke point covers
  startup load, reload, and replay's fresh-disk lookup. Ripple the fix
  plan missed: the JIDO.md drift-guard round-trip test mutates a literal
  phrase of the Custom Skills fragment ("capped at 256 bytes") to simulate
  drift — the phrase moved with the prose, so the mutation went no-op;
  the test now mutates "at most 256 bytes" (jido_md_test.exs joins the
  staged files).

- **Post-review round: test-compiled chaos seam replaces the fallback
  tautology** (forced; 2026-07-13). The static-fallback test decoded a
  JSON literal duplicated from `@static_fallback` and compared it to
  itself — `ErrorBoundary` never ran, and the guarded region's escape
  path is unreachable by input precisely because the production is total.
  `structured_item/1` now opens with `maybe_chaos!()` — compiled only in
  MIX_ENV=test (the `endpoint_config.ex` seam precedent; the project
  deliberately has no mocking library), reading `:error_boundary_chaos`
  with a catch-all so no configuration junk can arm it, and a constant
  no-op in production. The suite drives all three escape kinds
  (raise/throw/exit) through the real runtime and pins the exact static
  bytes, plus junk-config inertness and non-public-server scoping.

- **Cross-shape same-text signatures split under the identity contract**
  (forced; 2026-07-12). At HEAD, a transport-error envelope and a domain
  `isError` result carrying the same text collapsed into ONE text-only
  signature — `loop_guard_integration_test.exs` pinned a mixed 2+1 streak
  nudging on the 3rd. The plan's identity (`{code, message_prefix,
  details_fingerprint}`, code an explicit component + the §6 "different
  codes → distinct signatures" row) deliberately splits those shapes, so
  the incidental cross-shape merge cannot survive. The test's actual
  concern — an `isError` failure must never CLEAR the transport window —
  is preserved and re-asserted shape-locally (2 transport + isError
  interleave + 3rd transport → nudge). Detection for the mixed
  interleaving now needs 3 same-shape occurrences — accepted; the plan's
  "no previously-distinct signatures merge" invariant holds, and the
  inverse (previously-merged pairs may split) is the documented cost of
  the added distinction.

- **Post-review round: reserved wire state split from the extension bag
  (typed WireError)** (forced; 2026-07-13). Review round 1 found the
  collision class at both sites: a boundary-injected atom key beside a
  producer string twin of the same key encodes to ONE JSON key, and the
  bounded walker's collision arm replaces the stored value with
  `"[key-collision]"` — tier 3's `Map.put_new(:retry, …)` beside a
  producer `"retry"` hint (a supported jido_action shape) destroyed the
  wire's machine-readable retry boolean, and the registry fallback's
  `unregistered_code` injection beside a producer squatter destroyed the
  advertised closure-proof key. Round 2 rejected the round-1
  canonicalization-table draft as insufficient — `retry`/
  `unregistered_code` are boundary-owned protocol state living inside the
  producers' open extension bag, and every future reserved field would
  need three synchronized edits (canonicalization, reduction, minimal
  rendering). Landed shape: a typed boundary-internal `WireError`
  (`retry`/`truncated`/`unregistered_code` typed fields +
  `extra_details`), ONE build site for every tier with boolean-only
  extraction for `retry` AND `truncated` (atom-first, both key forms
  always dropped, junk ⇒ absent; `truncated` lifts the legitimate
  `Tools.Error.sanitize_details/1` producer signal and boundary
  reduction ORs its own `true` over it — trunc_meta merges LAST on both
  reduction tiers), an unconditional boundary-owned key strip
  (`unregistered_code` plus the `original_byte_size`/`observed_at_least`
  measurement keys, both forms — squatters can't counterfeit the
  fallback or the size metadata), fixed-lookup atom-first twin
  canonicalization for hint + `value`/`timeout` keys, and post-encode
  authoritative overlay onto encoded string keys; reduced/minimal tiers
  read the typed fields (`put_retry`/`put_unregistered` deleted). Wire
  deltas are junk-input-only, each pinned by a test (non-boolean
  reserved values abstain; inactive-fallback and size-metadata squatters
  no longer ship); NON-reserved twins keep the walker's sentinel —
  canonicalizing them would need exactly the unbounded key sweep the
  design refuses. Supersedes ONE #16 pin: §3's `Map.put_new` merge is
  replaced by `retry: Error.retryable?(error)` (entry below). The
  `to_map/1` adaptation itself was investigated and KEPT — a fixed-field
  replacement was drafted and REVERTED (the dep's default
  `:full`-telemetry span runs the same unbounded walk upstream so a
  boundary-side replacement cannot change path complexity, and parity
  broke: keyword/struct details normalization, sanitizer pid/ref
  renders, pseudo-message defaults).

- **Post-review round: details.retry defined as retry-policy eligibility**
  (forced; 2026-07-13). The exec gate is cap-first
  (`Jido.Exec.Retry.should_retry?/4`, retry.ex:72-80: attempt budget
  BEFORE `Error.retryable?/1`), so every error reaching the boundary is
  one the gate finally refused — "the wire reports what the gate did"
  was incoherent (constant `false`). The field is now defined as the
  CLASS component the gate ANDs with its budget, as downstream advice:
  `false` ⇒ not eligible for Jido.Exec's immediate automatic retry under
  the current failure classification (do not blindly repeat without
  intervention — deliberately NOT a determinism claim; a class-`false`
  `config_error` can succeed after an external configuration fix);
  `true` ⇒ the classification treats the failure as transient; ABSENT ⇒
  eligibility was not reported. `to_map/1`'s own `retryable?` field is
  NOT used — it diverges from the gate's predicate (`InternalError`
  hardcodes `false` where `retryable?/1` hint-defaults `true`;
  `ExecutionFailureError` diverges on the `:retryable` details-key
  spelling). Producer hints still win exactly where jido_action honors
  them (inside `retryable?/1`'s hint-folding). The definition is SERVED
  to clients via the new `ErrorCodes.retry_semantics/0` — a fourth
  bootstrap `error_contract` key plus the extended stability sentence
  riding `server_instructions` (compile-time concat, single-sourced;
  the unstaged v1.3 changelog bullet amended — nothing has shipped).
  Planned assertions survived with reframed semantics (class
  eligibility + the call-count proving the gate consulted the same
  predicate). `retryable?/1`'s own unbounded nested-reason hint walk is
  folded into the tier-3 dep-parity traversal residual — a bounded
  boundary-local reimplementation was rejected (it would reintroduce the
  gate-divergence risk choosing the dep predicate eliminates).

- **Post-review round: budget options normalized; key-prefix draws
  clamped under caller max_key_bytes** (forced; 2026-07-13). Review
  found `encode_bounded/2`'s public `:max_key_bytes` option broke the
  documented totality: both truncation helpers drew an unconditional
  256-byte prefix, so a sub-256 integer cap raised in `binary_part/3`
  (`max_key_bytes: 10` + a 20-byte key), a float cap survived the
  clamp's `min/2` and still raised, and a non-numeric cap (`:infinity`)
  defeated the over-cap comparison via Erlang term ordering — sending an
  oversized key into the full `String.valid?/1` scan the per-key budget
  exists to prevent. Two mechanisms: `limit_opt/3` normalizes all four
  budget options at BOTH public entry points (non-negative integers
  only; junk falls back to the pinned defaults), and `prefix_draw/2`
  clamps the tagged/truncated draws to the smaller of the caller's cap
  and 256, and to the key size. Defaults are byte-identical (both
  helpers are only reached when `byte_size > max_key_bytes`, and
  `min(256, 1024) = 256`); latent at HEAD (all in-repo callers use the
  defaults) but the option is public and the moduledoc promises
  totality.

- **Post-review round: compile_check regenerates the .app it cleans (a
  build-state-dependent gate flake)** (forced; 2026-07-13). Running the
  completion bar for the post-review fixes, `jidoclaw.jido_md.check`
  failed inside `mix precommit` with a BLANK expected version ("Version
  is 0.6.4, expected ") — while passing standalone. Root cause,
  reproduced deterministically: `jidoclaw.compile_check` runs `clean` +
  `compile.elixir`, which deletes `ebin/jido_claw.app` and never
  regenerates it (that is `compile.app`'s job), so a later same-session
  `Application.load(:jido_claw)` finds no spec — but ONLY when the build
  was already CURRENT at precommit entry (a stale entry triggers Mix's
  initial full compile, which regenerates the .app and loads the app
  first). The 2026-07-12 runs were green because the build was always
  stale at entry mid-implementation; a docs-only session entering with a
  current build trips it every time. Same artifact class compile_check
  already patches (`Mix.Project.build_structure/0` restoring the `priv`
  symlinks for later same-session steps): the fix adds
  `Mix.Task.rerun("compile.app", [])` beside it. Not caused by the
  post-review diff (neither the alias order nor either task is in its
  change set), but it blocks the gate, so the correction rides this
  change; `lib/mix/tasks/jidoclaw.compile_check.ex` joins the staged
  files.

- **Post-review round: bounded-walker leaf gaps closed (bignum width,
  calendar preflight); tier-4 reuses the single legacy render;
  end-to-end boundedness declared, not claimed** (forced; 2026-07-13).
  Same budget-contract class, in the walker's leaf handling:
  `bencode_value` passed numbers through uncharged, so an
  arbitrary-precision integer's full decimal was materialized by
  `Jason.encode!` before any cap fired, and the calendar clauses
  converted BEFORE any charge (a hand-built `%Date{year: bignum}`
  rendered unbounded; a non-ISO `calendar:` would dispatch arbitrary
  callbacks inside the structured branch). Fixes: (a) out-of-int64
  integers charge `2 × max(external_size − 8, 0)` — a provable LOWER
  bound of the decimal render (preserving `observed_at_least`'s
  contract) that is also tight: a single huge integer trips before
  Jason, and admitted bignums render within 2.625× of charge
  (`-(2^64 − 1)` is the worst ratio, 21 chars on an 8-byte charge; the
  ratio strictly decreases with magnitude). The review rejected a `− 16`
  header allowance because moderate bignums charged near-zero and
  aggregates slipped ~15× past the budget (40k × 10^30 ≈ 1.2 MB through
  a 128 KiB budget); finite-width numerics keep the documented
  node-bounded undercount (the minimal-tier test's mechanism —
  deviation above — survives untouched), and the boundary's
  `bound_retained` routes out-of-range integers to the small pass
  instead of the flat 8-byte charge. (b) The calendar clauses gain an
  O(1) fast-path gate — exactly `Calendar.ISO` + in-int64 temporal
  fields (microsecond both components) — with everything else taking
  the generic budgeted struct walk, so non-ISO calendar callbacks never
  dispatch in the bounded walker (a new documented
  `encode_bounded`-vs-`encode/1` divergence; `encode/1` keeps
  converting arbitrary calendars — totality, not bounded work, is its
  contract). (c) Tier 4's second unbounded `safe_inspect(reason)` is
  deleted: `error_response/2` computes the legacy render ONCE and
  threads it through `structured_item/2`; tier 4's message reuses those
  bytes (byte-identical by construction — same function, same input),
  pinned by a counting-Inspect fixture (`HostileInspect.Counting` —
  exactly one dispatch across both content items; text equality alone
  would pass under a second render), and `bound_native_message/1` stops
  `to_map`'s `limit: :infinity` MESSAGE inspect (the one dep arm
  boundable without parity loss). (d) The end-to-end guarantee is
  NARROWED AND DECLARED (the reviewer's declare-and-narrow option,
  chosen over unpinning the v1.3 `content[0]` byte-identity contract —
  a static tier-4 message alone buys nothing while content[0] still
  renders; reuse is strictly better): the served call is not
  resource-bounded against hostile reasons — content[0]'s pinned legacy
  inspect (once per call), the dep's `:full`-telemetry `to_map` walk,
  per-attempt `cond_log_error` renders when error logging is on, and
  tier 3's dep-parity `to_map` (a traversal-AND-rendering residual:
  transient sort-key renders for detail keys, `safe_inspect` for
  unsupported values, beside `retryable?/1`'s hint walk) — while the
  boundary's own machinery adds only budgeted work, with the
  single-render pin scoped to boundary content production and the
  promptness tests boundary-scoped by design.

- **Post-review round: tier-1 wrap provenance witnessed through a
  Jido.Exec fork** (forced; 2026-07-13). A review round after the
  whimsical-sunrise fixes validated ONE finding: tier 1 inferred "the
  exact `Jido.Exec` wrap of a canonical envelope" from an ambiguous
  native-error SHAPE — exec passes exception structs through VERBATIM,
  and `ExecutionFailureError`'s details map is unrestricted, so a
  hand-built native error carrying `:code`+`:details` mis-tiered into
  tier 1 (wire reported the nested code as the domain code, retry from a
  producer hint instead of `retryable?/1`). No shape refinement can close
  it — the wrap is lossy; post-wrap a canonical envelope and a
  shape-colliding term are byte-identical. Mechanism shipped: the two
  map-wrap arms of a forked exec stamp an opt-provided per-call ref —
  ...
  struct clause); `Map.put_new` (squatters keep their junk, bytes stay
  faithful, mis-tier is always DOWN); detach on exact ref identity before
  the legacy render; tier 1 requires the witness; public-server-only
  minting. Junk-input-only wire deltas accepted (colliding structs +
  near-envelopes demote to `execution_error`). Rejected alternatives:
  producer-side envelope marking (silently demotes every non-pipeline
  envelope to tier 4), an envelope exception type (breaks `content[0]`
  byte identity + reclassifies the exec retry gate), telemetry
  correlation (jido_action 2.x has NO seam — `:error_normalization` is
  accepted-and-ignored, span metadata never carries the pre-wrap reason),
  a static marker (forgeable, can't restore bytes under squatting).
  Process: the fork is a fidelity-critical port, so
  `docs/exploration/pms/pad/PORT-PD1-2-EXEC.md` (an amendment map
  anchored to the PD1-2 corpus entry — plan review overruled the initial
  header-convention-only reading of the port rule) was written FIRST and
  explicitly signed off 2026-07-13 before any fork code. The fork is a
  compile-time GENERATOR patching the pinned upstream source, not a
  committed copy — a plan-review round showed a committed copy shifts
  `handle_action_exception`'s embedded stacktrace frames and breaks
  no-opt raise-path byte identity; the generator compiles under the
  dep's own recorded file string, so parity and frame fidelity hold by
  construction (implementation confirmed the frames carry the
  DEP-ROOT-RELATIVE `lib/jido_action/exec.ex`, not the absolute
  compile_info path — the generator passes exactly that string).
  Implementation also surfaced one owner bug the tests caught: Erlang
  SPLICES list-valued persisted attributes, so the build marker reads
  back FLAT from `:beam_lib` — the single-wrap pattern would have made
  the incremental-build owner regenerate every compile and never verify.
  Drift guards: BOTH packages pinned (mix.lock version + outer checksum;
  jido_mcp's pin retrofits the guard the pre-existing runtime fork never
  had), the exec source sha compile-time gated, the runtime fork
  parity-reconstructed from upstream + enumerated transformations, and
  the generated BEAM owned every compile via a dual-digest marker
  (upstream sha + patch revision). Licensing: Apache-2.0 §4 — committed
  license byte-copy + NOTICE in `priv/licenses/`, escript-embedded via
  `ThirdPartyLicenses` behind the pre-boot `--third-party-licenses`
  flag; the five PRE-EXISTING patches' license hygiene is a flagged
  follow-up, not absorbed here. The bare-binary license route forced the
  escript boot-order chain: `app: nil` + checked `start_app_or_halt!/0`
  on every booting branch (the three discarded `ensure_all_started`
  results were a latent fall-through under `app: nil`) + a TOTAL
  runtime.exs — the redundant CLOAK_KEY cipher block DELETED in favor of
  the existing `VaultConfig` startup loader after review flagged the
  raw-passthrough key-collision hazard (base64 of 16/24-byte keys is
  itself 24/32 bytes), the dead `FORGE_SANDBOX_TIMEOUT_MS` config
  REMOVED rather than relocated (activating it would be a separate
  behavioral change needing a call-site inventory — its
  `default_timeout_ms` key has no consumer; Docker calls without an
  explicit timeout use `:infinity`), the secret raises moved behind
  `RuntimeSecrets` (which already enforced presence+quality at startup —
  the runtime.exs raises were redundant AND too early), and POOL_SIZE
  parsing relocated to `Repo.init/2` chaining `super/2`.

- **Artifact probes surfaced four escript-only defects (three
  pre-existing)** (forced; 2026-07-14). The plan's artifact-level
  verification (a MIX_ENV=prod escript driven end-to-end) caught what no
  mix-run test could: (1) the escript's stdout device is latin1-mode, so
  `IO.write` escaped the NOTICE's UTF-8 em-dash into literal `\x{2014}` —
  fixed with `IO.binwrite` + a deliberately pure-ASCII NOTICE (the
  capture_io byte-equality row now fails loudly on any future non-ASCII
  edit, since a unicode capture device latin1-reinterprets raw bytes);
  (2) PRE-EXISTING: `DependencyPatches.load_module!` probed candidates
  with `File.exists?`, which cannot see into the escript archive — every
  escript boot since the first patch landed crashed in
  `Application.start`; fixed with an archive-aware fallback
  (`Code.ensure_loaded/1` — sound because prod relocation leaves exactly
  ONE, patched, copy per module in the archive; still loud when genuinely
  absent; dev-built escripts remain out of contract); (3) PRE-EXISTING:
  Elixir's `:logger` app boots DURING `ensure_all_started` on the
  escript's `app: nil` paths and re-installed its default handler on
  standard_io, clobbering the earlier redirect and corrupting the
  JSON-RPC stream with dep boot logs — `redirect_logger_to_stderr/0` now
  also writes the `:logger, :default_handler` app env so the later boot
  lands on stderr (no config file sets that key); (4) PRE-EXISTING:
  `Application.put_env` before the app is LOADED is clobbered by
  `Application.load`'s app-spec env — the MCP branch's non-persistent
  `:mode`/`:serve_mode` left the Phoenix endpoint starting inside MCP
  mode and the MCP server itself not starting; every pre-boot put_env in
  `CLI.Main` now passes `persistent: true`. (2)–(4) were invisible
  before this slice only because the escript crashed at boot outright;
  under the old pre-`main/1` bootstrap the same defects applied. The
  probes then passed in full: bare-env license print byte-identical,
  all three malformed-parser probes total, the MCP stdio smoke clean
  (valid JSON-RPC only on stdout; the failing `tools/call` returned the
  dual-content shape with the DOMAIN code in content[1] — the witnessed
  tier-1 path proven in the artifact), and the scrubbed secrets-unset
  boot failed with exit 2 naming `RuntimeSecrets`.

- **Post-review round 3: compile_check fork regeneration + escript run
  persistent env** (forced; 2026-07-14). A second post-review pass
  reported two P1s, both validated. (1) `mix jidoclaw.compile_check`
  hand-sequences `clean` → `compile.elixir` → `compile.app`, so the
  mix.exs `compilers/0` ordering never applied inside the gate: `clean`
  deleted the app-side generated `Jido.Exec` fork BEAM, `compile.elixir`
  never runs custom compilers, and `compile.app`'s ebin scan wrote a
  `jido_claw.app` without the module — a post-gate no-recompile boot
  fell back to the upstream dep BEAM and silently lost the tier-1
  provenance witness (`mix precommit` masked it end-to-end only because
  the partitioned test phase spawns fresh mix sessions whose full
  compile re-runs the compiler). Fixed by rerunning
  `compile.jidoclaw_release_patches` between `compile.elixir` and
  `compile.app` and SELF-ASSERTING the artifact at the end
  (`assert_exec_fork!/2` — marker-current BEAM + `.app` membership at
  the env's owner: app ebin in dev/test, the relocated `jido_action`
  ebin plus the single-owner invariant in prod). Coverage decision
  (operator-approved): the self-assert executes the real sequence on the
  real build on every precommit — strictly more frequent than a literal
  subprocess integration test (~1–2 min per suite run) — plus fast
  path-parameterized fixture rows for the assert helper. Plan-review
  amendment: the fork's `Code.compile_string` warnings were
  print-at-best — `compile_with_diagnostics/2` now captures them, the
  compiler task returns them on the proper `Mix.Task.Compiler` contract,
  and the gate runs BOTH diagnostic sources through one allowlist split.
  (2) The escript `run` path predates the `app: nil` flip: `boot/2`'s
  plain pre-boot `put_env`s were clobbered when `ensure_all_started`'s
  `Application.load` re-applied the spec/persistent embedded config
  (Mix's escript main applies it `persistent: true` before `main/1`) —
  the one-shot binary booted Phoenix (`mode: :both`) and config-default
  models; run_command_test's no-op boot stub could never expose it.
  Fixed by `prime_boot_env/2` (load the app specs FIRST, then plain
  overrides). Second plan-review amendment: load-then-put chosen over
  main.ex's `persistent: true` pattern because prime also runs inside
  the test VM on every run-command row, where a persistent shadow would
  outlive the non-persistent on_exit restore. Scope honesty: only the
  config-defined `mode`/`model_aliases` pair was actually clobbered
  (`Application.load/1` preserves pre-load keys the spec/persistent set
  doesn't define, so `project_dir`/`skip_discord` survived); the
  subprocess regression seeds those two adversarially and asserts all
  four primed values as a deliberately stronger invariant.
