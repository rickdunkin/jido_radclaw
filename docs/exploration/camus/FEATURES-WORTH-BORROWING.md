...
**Note (2026-07-07)**: the harness feature camus is *built on* now has its own corpus subject — `docs/exploration/cc-dynamic-workflows/` (first non-repo subject: the announcement post + firsthand harness operation). This doc's engine-half verdict ("composer + Reactor already superset it") extends there from camus's *use* of the feature to the feature itself, with the architectural why recorded (their script double-duties as control flow + durability unit; ours splits them). Its DW-2/DW-3 program grafts the one capability the table below marks "Nothing" for — model-authored harnesses — onto our engine, and its DW-4 cites the subject's `budget.spent()` pool as the mechanism our count-denominated budgets lack.
