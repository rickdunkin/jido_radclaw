...
- `Orchestration.RunPubSub` topics are defined but no code publishes to them.
...
| `workflows:run:<id>` | `Orchestration.RunPubSub` (exists, no publishers) | Wire up the runner to publish |
...
1. **Finish the workflow runner** — wire `plan_workflow.ex` to actually persist `WorkflowRun`/`WorkflowStep` and broadcast on `RunPubSub`. Unblocks every "view workflows" feature with zero new resources.
...
**Live broadcast:**
- `Orchestration.RunPubSub` — `orchestration/run_pubsub.ex:1-17`. Defines topics `orchestration:run:<id>` and `orchestration:runs`. `RunSummaryFeed` GenServer subscribes.
- **Zero callers of `RunPubSub.broadcast/2`.** Confirmed at `dashboard_live.ex:79`: comment reads "RunPubSub — not yet broadcast".
...
- `orchestration:run:<id>`, `orchestration:runs` — defined, no publishers.
