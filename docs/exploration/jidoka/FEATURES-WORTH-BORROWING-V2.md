...
**Deferred (keeps this PARTIAL)**: per-template allowlist enforcement for `mcp_*` tools — the shared enabler (threading `:agent_template` into spawned / follow-up / step `ToolContext`s) **shipped 2026-06-13** with V2-1's per-template `require_approval`, but the MCP approval path (`mcp_requirement/2`) resolves *before* the native template overlay (`require_approval` is native-tools-only by design), so a template cannot yet gate or allowlist individual external tools beyond the per-server `require_approval` default; worker/sub-agent sync; generic MCP output shaping (proxies redact + cap but do not format-shape/`fetch_output`-store); reconnect/re-discovery — including **no auto re-prep after a hard prep crash** (a hard-killed prep transitions the Consumer to `:failed`/tool-less, fail-closed policy, until an app restart re-preps).
