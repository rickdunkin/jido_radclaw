...
3. **V2-3 and V2-5** are independent and opportunistic; neither justifies a dedicated work item today. **V2-7** is a watch entry. (**V2-4** and **V2-6** were opportunistic borrows that have since shipped.)

**Program status (2026-06-15)**: the borrows that warranted dedicated work are all shipped — **V2-1** approval gate (`7fa6267`, `28a01ce`), **V2-2** external MCP consumption (`3cde549`, `ce96f02`), **V2-4** replay preflight diagnostics (`e8704be`), and **V2-6** web search (the `search_web` tool wrapping `Jido.Browser.Actions.SearchWeb`) — so the active Jidoka-V2 borrowing program is complete. (V2-1 and V2-2 keep their PARTIAL labels only for deliberate out-of-scope deferrals — V2-1's input/output-length controls are out of the threat model, and V2-2's per-tool approval overlay is superseded by the per-template reach-allowlist; neither is pending work.) The remainder are intentional standing deferrals per their entries: **V2-3** (trace split) and **V2-5** (eval harness) are opportunistic-if-touched, and **V2-7** (Lua DAGs) is a watch entry. Nothing is secretly half-done; the two upstream watch triggers below remain the only re-audit prompts.
