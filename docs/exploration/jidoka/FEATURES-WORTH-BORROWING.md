...
- **Durability counters across dispatch targets** — `run_count`/`last_run_at` stamped after every tick for any persisted job (agent/workflow/mfa) inside the fenced `record_success`/`record_failure` outcome writes (the standalone `:record_run` action they superseded is deleted), best-effort so a DB hiccup never crashes the worker. (Durability = per-job counters across dispatch targets, not per-kind.)
