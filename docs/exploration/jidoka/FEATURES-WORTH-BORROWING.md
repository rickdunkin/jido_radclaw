...
**Status (2026-05-28)**: PARTIAL — the data layer is fully built and in real use, but the cross-surface unification that is the feature's whole point is only partly realized. The `%JidoClaw.AgentView{}` struct + `snapshot/2` + `to_mcp_map/1` are complete (read-only **session-axis** projection of "what is this agent doing right now," aggregating `JidoClaw.Trace`, `JidoClaw.Session.Worker`, `JidoClaw.Agent.Handoff.Registry`, and optionally `JidoClaw.Reasoning.Compactor.Storage`) and consumed by **two of five** surfaces — the rewired `agents_live.ex` LiveView and the new `agent_status` MCP tool. The CLI REPL, `dashboard_live`, and `forge_live` still hand-roll their own projections and are deferred, so the drift this feature targets persists there. The macro, lifecycle callbacks, and `streaming_message` are deliberate non-goals/placeholders — not gaps (see divergences); what keeps this PARTIAL rather than ADOPTED is that the remaining surfaces aren't yet covered.

**Correction (2026-05-29)**: "unfinished consumer migration" overstated the remaining path. The three unmigrated surfaces project *different subsystems* than the per-session `AgentView` models — the CLI REPL renders per-agent swarm state via `AgentTracker`, `forge_live` renders Forge sandbox harnesses, and `dashboard_live` renders workflow-run aggregates. Pointing them at the session-axis `AgentView` would lose data, not unify it; taking T2-2 to ADOPTED is a multi-axis view *redesign* (a broader shape that also models the swarm / Forge / workflow axes), not a mechanical migration of consumers onto an existing shape. T2-2 stays PARTIAL, but the gap is "AgentView doesn't model those axes yet," not "the consumers haven't been pointed at it."
...
**Where in jidoka**: `lib/jidoka/subagent.ex`, plus `lib/jidoka/capability/subagent/{context,definition,metadata,tool}.ex` and `lib/jidoka/capability/subagent/runtime/{calls,executor,result,trace}.ex` (moved from `lib/jidoka/subagent/*` since the 2026-05-18 baseline). The full subagent shape there stays SUPERSEDED by swarm (`Tools.SpawnAgent` + `Tools.GetAgentResult` + `AgentTracker` + worker modules — real OTP processes with bidirectional messaging); only the `forward_context` knob was the genuine gap, and it's now closed.
...
- **T2-2 AgentView** — PARTIAL 2026-05-28. Session-axis data layer (`snapshot/2` + `to_mcp_map/1`) done and consumed by `agents_live` + `agent_status`. The remaining surfaces (REPL, `dashboard_live`, `forge_live`) project *different subsystems* (per-agent swarm via `AgentTracker`, Forge harnesses, workflow-run aggregates), so taking it to ADOPTED is a multi-axis view redesign, not a mechanical consumer migration.
