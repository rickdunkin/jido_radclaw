...
```sh
mix usage_rules.search_docs "search term" -p ash -p ash_admin -p ash_archival -p ash_authentication -p ash_authentication_phoenix -p ash_cloak -p ash_json_api -p ash_paper_trail -p ash_phoenix -p ash_postgres -p ash_state_machine
```
